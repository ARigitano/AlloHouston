...
namespace Assets.ManusVR.Scripts
{
    ...
    public struct ApolloSuccessPayload
    {
        ...
        public string text { get{ return Marshal.PtrToStringAuto(pointer); } }

        public UInt64 uint64 { get { return (UInt64)Marshal.ReadInt64(pointer); } }
    }
    ...
    public struct ApolloRawData
    {
        ...
        public double flex(int finger)
        {
            ...
            if (finger > 9 || finger < 0)
                Debug.LogError(string.Format("Finger should be between 0 and 9, got: {0}", finger));
            else
            {
                flexValue = BitConverter.ToDouble(flexRaw, finger * sizeof(double));
                // verify the conversion
                if (flexValue < 0.0D || flexValue > 1.0D)
                    Debug.LogWarning(string.Format("flex sensor [{0}] value {1} is not between 0 and 1", finger, flexValue));
            }
