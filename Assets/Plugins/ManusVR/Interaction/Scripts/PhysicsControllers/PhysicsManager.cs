...
using System.Linq;
using Assets.ManusVR.Interaction;
using UnityEditor;
using UnityEngine;
...
namespace Assets.ManusVR.Scripts.PhysicalInteraction
{
    ...
    public class PhysicsManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (editorPhysicsPreferences.ShouldPromptFixedTimestep && Time.fixedDeltaTime > editorPhysicsPreferences.SuggestedTimestep)
            {
                if (EditorUtility.DisplayDialog("Incorrect Timestep Settings",
                    "We've found that setting the Fixed Timestep to at least " + editorPhysicsPreferences.SuggestedTimestep +
                ...
                {
                    editorPhysicsPreferences.ShouldPromptFixedTimestep = false;
                    AssetDatabase.Refresh();
                    EditorUtility.SetDirty(editorPhysicsPreferences);
                    AssetDatabase.SaveAssets();
                }
                else
                {
                    EditorGUIUtility.systemCopyBuffer = editorPhysicsPreferences.SuggestedTimestep.ToString();
                }
                EditorApplication.isPlaying = false;
                return;
            }

            if (editorPhysicsPreferences.ShouldPromptGravitySettings && !FloatComparer.AreEqual(Physics.gravity.y, editorPhysicsPreferences.SuggestedGravityForce, 0.0001f))
            {
                if (EditorUtility.DisplayDialog("Gravity Settings",
                    "We've found that setting the gravity to " + editorPhysicsPreferences.SuggestedGravityForce +
                    " results in the most accurate physics interactions. \n\n" +
                    "Consider adjusting your gravity settings in: Edit/Project Settings/Physics.",
                    "Don't Remind Me Again!",
                    "I Will,\n Copy Value To My Clipboard!"))
                {
                    editorPhysicsPreferences.ShouldPromptGravitySettings = false;
                    AssetDatabase.Refresh();
                    EditorUtility.SetDirty(editorPhysicsPreferences);
                    AssetDatabase.SaveAssets();
                }
                ...
                {
                    EditorGUIUtility.systemCopyBuffer = editorPhysicsPreferences.SuggestedGravityForce.ToString();
                }
                EditorApplication.isPlaying = false;
                return;
            }

            if (editorPhysicsPreferences.ShouldPromptDefaultSolverIterations && Physics.defaultSolverIterations != editorPhysicsPreferences.SuggestedDefaultSolverIterations)
            {
                if (EditorUtility.DisplayDialog("Default Solver Iterations",
                    "We've found that setting the amount of Default Solver Iterations to " + editorPhysicsPreferences.SuggestedDefaultSolverIterations +
                    " results in the most accurate physics interactions. \n" +
                    "Consider adjusting your Default Solver Iterations Settings in: Edit/Project Settings/Physics.",
                    "Don't Remind Me Again!",
                    "I Will,\n Copy Value To My Clipboard!"))
                {
                    editorPhysicsPreferences.ShouldPromptDefaultSolverIterations = false;
                    AssetDatabase.Refresh();
                    EditorUtility.SetDirty(editorPhysicsPreferences);
                    AssetDatabase.SaveAssets();
                }
                ...
                {
                    EditorGUIUtility.systemCopyBuffer = editorPhysicsPreferences.SuggestedDefaultSolverIterations.ToString();
                }
                EditorApplication.isPlaying = false;
                return;
            }

            if (editorPhysicsPreferences.ShouldPromptDefaultSolverVelocityIterations && Physics.defaultSolverVelocityIterations != editorPhysicsPreferences.SuggestedDefaultSolverVelocityIterations)
            {
                if (EditorUtility.DisplayDialog("Default Solver Velocity Iterations",
                    ...
                    "I Will,\n Copy Value To My Clipboard!"))
                {
                    editorPhysicsPreferences.ShouldPromptDefaultSolverVelocityIterations = false;
                    AssetDatabase.Refresh();
                    EditorUtility.SetDirty(editorPhysicsPreferences);
                    AssetDatabase.SaveAssets();
                }
                ...
                {
                    EditorGUIUtility.systemCopyBuffer = editorPhysicsPreferences.SuggestedDefaultSolverVelocityIterations.ToString();
                }
